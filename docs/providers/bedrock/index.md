...
keywords: docker agent, ai agents, model providers, aws bedrock
---
...
> [!TIP]
> **Inference profiles**: Use `global.` prefix on model IDs for automatic cross-region routing. Use `eu.` prefix for GDPR compliance.
...
> [!NOTE]
> **Temperature and top_p**: Bedrock Claude suppresses `temperature` and `top_p` while extended thinking is active — Anthropic requires `temperature=1.0` internally.
